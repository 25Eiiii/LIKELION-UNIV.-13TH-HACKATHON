...
const Chatbot = () => {
  ...
  return (
    <C.Container>
        ...
        <C.ChatbotName>{name}</C.ChatbotName>
        <C.QuestionWrapper>
        ...
        <C.Recommend>
            <C.RecText>
                <p style={{margin: "0px"}}>
                효민님 지난 달 참여하였던<br /> <span style={{ color: "#159054" }}>피카소 전시 따라잡기</span> 와 비슷한 행사를 추천해드릴게요
                </p>
            </C.RecText>
            <C.RecList>
                {recs.map((event, idx) => (
                    <EventCard key={idx} {...event} w={107} h={154}></EventCard>
                ))}
            </C.RecList>
        </C.Recommend>
        <C.SendWrapper>
